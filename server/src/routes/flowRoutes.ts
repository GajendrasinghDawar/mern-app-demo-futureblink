...
flowRouter.post(
  '/flows/save',
  async (
    req: Request<Record<string, never>, SaveFlowResponse | { error: string }, SaveFlowRequestBody>,
    res: Response,
  ) => {
    try {
      ...
      return res.status(201).json({ id: String(saved._id), createdAt: saved.createdAt.toISOString() });
    } catch (error) {
